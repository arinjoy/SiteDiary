...
final class DiaryEntryNetworkingURLMockTests: XCTestCase {
    ...
    private let resource = Resource<DiaryEntry>.create(TestHelper.sampleDiaryEntry)
    ...
    func testURLLoadingSuccess() {
        ...
        URLProtocolMock.requestHandler = { _ in
            ...
            return (response, TestHelper.jsonData(forResource: "diary_entry_valid"))
        }

        // WHEN
        networkService.save(resource)
        ...
        XCTAssertEqual(response.images[0], "string1")
        XCTAssertEqual(response.images[1], "string2")
        XCTAssertEqual(response.comments, "some comment")
    }

    func testURLLoadingFailure500ServerError() {
    ...
    func testURLLoadingFailure403ForbiddenError() {
        var resultingError: NetworkError!
